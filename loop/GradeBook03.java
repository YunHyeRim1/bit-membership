...
=======
// 메인 메소드가 모든 걸 담당하지 않고
// 개별 메소드가 한 가지 담당만 하는 버전
...
=======
    // 파라미터의 데이터타입에 따라서
    // call by value 와 call by reference 가 발생하게 된다.
    // call by value: 변수를 넘겨주더라도 값만 넘어가고 변수 그 자체는 넘어가지 않는다.
    // call by reference: 주소값을 넘겨주기 때문에 변경을 가하면 변수에도 변경이 가해진다.
    
    // call by value 의 경우, 해당 변수의 "값 복사본"이 넘어가고,
    // 따라서 파라미터에 값을 변화시키더라도 원본 변수의 값은 변하지 않는다.
    
    // call by reference 의 경우, 해당 변수의 "주소값"이 넘어가기 때문에
    // 메소드에서 파라미터를 변화시키면 원본 변수에도 해당 변화가 적용된다.
    
    // call by value 는 기본형 데이터타입 변수가 파라미터로 넘어갈 경우 발생하고
    // call by reference 는 참조형 데이터타입 변수가 파라미터로 넘어갈 때 발생한다.
>>>>>>> 02bb71477c4c8cdccfe81c1929f0e810d1379a8e
