 ...
 *     필요한 프로그램을 만드는 것을 객체지향 프로그래밍이라고 한다.
 *     클래스라는 것 자체가 한 가지 책임에 따라 거기에 알맞은 정보를 넣는 칸과 기능들이 구현되어
 *     있기 때문에 필요한 클래스 변수, 즉 객체를 만들어서 프로그램에서 사용하게 되는 것이다.
 *  
 *  3. 클래스의 구성요소
 ...
 *     데이터타입 필드이름
 *     으로 선언한다.
 *     실제 어떤 데이터가 들어갈지는 해당 객체를 사용할때 지정해주면 된다.
 *     
 *  5. 메소드
 ...
 *     따라서, 파라미터 있는 생성자와 파라미터 없는 생성자 모두 필요하다면
 *     둘다 만들어야 한다.
 *     만약 해당 객체가 외부로부터 어떠한 값을 반드시 받아와야 한다면
 ...
 *     A. String string = "";
 *        이 코드는 String 클래스 객체 string이 있고, 아무런 문자도 안 들어가있지만
 *        그 외에 메소드나 공간은 확보가 되어있는 상태이다. 
 *        즉, NullPointerException은 발생하지 않는다.
 *     B. String string = null;
 *        이 코드에서는 String 클래스 객체 string이 선언 되었고 heap 영역의 메모리주소값까지는
 *        부여가 되었지만, 실제 해당하는 heap 영역의 메모리는 아직 초기화가 안 돼서 String 클래스에
 *        관한 내용이 하나도 없는 상태이다.
 *  
 *  9. java.lang.Object
 *     Object 클래스는 자바가 기본적으로 제공해주는 클래스인데 
 *     자바가 기본적으로 제공하든 아니면 누군가가 만들었든 뭐든 자바의 존재하는 모든 클래스는
 ...
 *     반드시 해당 메소드들을 '재정의' 해주어야 원하는 결과를 얻을 수 있다.
 *     
 ...
 *        만약 해당 클래스의 정보 중 위의 정보가 아닌 필드의 값과 관련한 정보를
 *        String으로 뽑아내야한다면 당연히 toString() 메소드를 오버라이드 해야 한다.
 *     B. equals()
 ...
 *        만약 여러분들이 두 객체의 비교를 해야 할 일이 있다면(=데이터담당 클래스일 경우)
 *        반드시 equals를 오버라이드 해야 한다.
 ...
 *      모두다 private 접근제한자를 적용시켜서 외부에서는 이 클래스 객체 안의 내용을 알 수 없게 만드는
 *      데이터 은닉화의 방법이다.
